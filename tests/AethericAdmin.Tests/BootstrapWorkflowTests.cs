...
using App = AethericAdmin.BootstrapShell.BootstrapApp;
...
public sealed class BootstrapWorkflowTests
{
    ...
    public async Task Verified_connection_opens_account_form_without_human_oidc_login()
    {
        ...
        Assert.Null(host.App.Services.GetService<AethericAdmin.Web.Maintenance.MaintenanceDispatchService>());
        Assert.Null(host.App.Services.GetKeyedService<StackExchange.Redis.IConnectionMultiplexer>("Workbench"));
        Assert.Equal(HttpStatusCode.NotFound, (await host.Client.GetAsync("/maintenance")).StatusCode);
        Assert.Equal("/setup", (await host.Client.GetAsync("/")).Headers.Location!.ToString());
    ...
    private sealed class Host : IAsyncDisposable
    {
        ...
        public static async Task<Host> Start(bool useDefaultOrigin = false, bool existingAccounts = false)
        {
            ...
            {
                ["BootstrapConnection:Authority"] = host.Configuration.Authority,
                ["BootstrapConnection:Realm"] = host.Configuration.Realm,
                ["BootstrapConnection:ClientId"] = host.Configuration.ClientId,
                ["BootstrapConnection:PublicOrigin"] = host.Configuration.PublicOrigin,
                ["BootstrapConnection:StateDirectory"] = host._directory,
                ["RootCredentials:Directory"] = Path.Combine(host._directory,"credentials"),
                ["RootCredentials:KeyDirectory"] = Path.Combine(host._directory,"root-key"),
                ["Bootstrap:ProtectionKeyDirectory"] = Path.Combine(host._directory,"protection")
            });
            ...
            if (existingAccounts)
            {
                ...
                builder.Services.RemoveAll<ISetupRegistryClients>();
