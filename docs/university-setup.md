...
The pinned runtime currently implements the older single-Campus command. A University submission adapter must wait for the runtime contract, durable queue acceptance/idempotency semantics, credential resolution, and job/result tracking. The disabled submission control intentionally does not fabricate success or send this draft to the Campus consumer. Likewise, the existing recurring/SSH maintenance job dispatcher is not used as a substitute.
